      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-select v-model="searchParams.questionType" placeholder="选择题型" style="width: 120px; margin-right: 10px;">
            ...
            <el-select v-model="searchParams.difficulty" placeholder="选择难度" style="width: 120px; margin-right: 10px;">
              <el-option label="全部难度" value="" />
              <el-option label="容易" value="容易" />
              <el-option label="中等" value="中等" />
              <el-option label="困难" value="困难" />
            </el-select>
            <el-button type="danger" @click="handleBatchDelete" :disabled="selectedQuestionIds.length === 0">
      ...
      >
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="scope">
            <el-button type="primary" size="small" @click="handleEditQuestion(scope.row)">
              <el-icon><Edit /></el-icon>
              编辑
            </el-button>
            <el-button type="danger" size="small" @click="handleDeleteQuestion(scope.row.questionId)">
              <el-icon><Delete /></el-icon>
              删除
            </el-button>
          </template>
      ...
      <el-form :model="questionForm" label-width="120px">
        ...
        <el-form-item label="所属知识点" required>
          <el-select v-model="questionForm.pointId" placeholder="请选择知识点">
            <el-option v-for="point in knowledgePoints" :key="point.pointId" :label="point.pointName" :value="point.pointId" />
          </el-select>
        </el-form-item>
        <el-form-item label="题目内容" required>
          <el-input v-model="questionForm.content" type="textarea" placeholder="请输入题目内容" :rows="4" />
        </el-form-item>
        
        <!-- 选项（仅客观题显示） -->
        <el-form-item label="选项" v-if="['单选题', '多选题'].includes(questionForm.questionType)">
          ...
          <el-button type="primary" size="small" @click="addOption" v-if="options.length < 10">
            <el-icon><Plus /></el-icon>
            添加选项
          </el-button>
        </el-form-item>
        ...
        <el-form-item label="正确答案" required>
          <el-input 
            ...
            placeholder="请输入正确答案" 
            v-if="['单选题', '填空题', '简答题'].includes(questionForm.questionType)"
          />
          <el-checkbox-group v-model="correctAnswers" v-else-if="questionForm.questionType === '多选题'">
            <el-checkbox 
          ...
          <el-radio-group v-model="questionForm.answer" v-else-if="questionForm.questionType === '判断题'">
            <el-radio label="对" />对
            <el-radio label="错" />错
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="分值" required>
          <el-input v-model.number="questionForm.score" placeholder="请输入分值" type="number" min="0.5" step="0.5" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const handleEditQuestion = (row) => {
  ...
  if (['单选题', '多选题'].includes(row.questionType)) {
    options.value = row.options ? JSON.parse(row.options) : [{ content: '' }, { content: '' }]
  }
...
const handleSaveQuestion = async () => {
  try {
    ...
    if (['单选题', '多选题'].includes(questionForm.questionType)) {
      questionForm.options = JSON.stringify(options.value.filter(opt => opt.content))
    }
...
const resetQuestionForm = () => {
  Object.assign(questionForm, {
    ...
    pointId: knowledgePoints.value.length > 0 ? knowledgePoints.value[0].pointId : null,
    questionType: '单选题',
    content: '',
    options: '',
    answer: '',
